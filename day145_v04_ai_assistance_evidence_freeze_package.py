"""Day145 v0.4 AI assistance evidence freeze package.

This module freezes the existing Day127-Day144 AI assistance evidence range as
local-only, deterministic reviewer evidence. It does not rerun, rewrite, repair,
or modify Day144 outputs and does not call providers, APIs, OpenAI API, models,
runners, adapters, SSH, NETCONF, RESTCONF, RouterOS, live devices, or cleanup
paths.
"""

from __future__ import annotations

from copy import deepcopy
import hashlib
import html
import json
from pathlib import Path
from typing import Any, Callable, Dict, Iterable, Mapping, Optional, Tuple


DAY = 145
DAY_LABEL = "Day145"
TASK_NAME = "v0.4-ai-assistance-evidence-freeze-package"
TITLE = "v0.4 AI Assistance Evidence Freeze Package"
FULL_TITLE = f"{DAY_LABEL} {TITLE}"
MODE = "REVIEW_ONLY_EVIDENCE_FREEZE"
OVERALL_STATUS = "PASS"
FAIL_STATUS = "FAIL"
READY_STATUS = "V0_4_AI_ASSISTANCE_EVIDENCE_FREEZE_READY"
BLOCKED_STATUS = "V0_4_AI_ASSISTANCE_EVIDENCE_FREEZE_BLOCKED"
FINAL_RECOMMENDATION = "FREEZE_DAY127_DAY144_V0_4_AI_ASSISTANCE_EVIDENCE_KEEP_NEXT_PHASE_FALSE"
FREEZE_COMMIT_HASH = "ddefb46e045df5310634ad307937f81c9f08e6cb"
REPORT_JSON = Path("reports") / "lab-summary" / "day145_v04_ai_assistance_evidence_freeze_package.json"
REPORT_HTML = Path("reports") / "lab-summary" / "day145_v04_ai_assistance_evidence_freeze_package.html"
ROADMAP_DOC = Path("docs") / "roadmap" / "day145_v04_ai_assistance_evidence_freeze_package.md"
AI_INTENT_DOC = Path("docs") / "ai-intent" / "day145_v04_ai_assistance_evidence_freeze_package.md"

INITIAL_GIT_STATUS_OBSERVED = "## main...origin/main"
RECENT_COMMITS_OBSERVED: Tuple[Dict[str, str], ...] = (
    {"hash": "ddefb46", "subject": "Add Day144 v0.4 AI assistance compatibility review"},
    {"hash": "e17e5dd", "subject": "Add Day143 dry-run draft safety diff viewer"},
    {"hash": "22b3ad7", "subject": "Add Day142 AI summary to dry-run draft display contract"},
    {"hash": "feabaff", "subject": "Fix Day141 task catalog metadata"},
    {"hash": "fcb322e", "subject": "Add Day141 AI assistance review demo package"},
    {"hash": "d894f07", "subject": "Add Day140 folder move compatibility gate"},
    {"hash": "557982c", "subject": "Add Day139 docs-only move dry-run evidence plan"},
    {"hash": "ea740cb", "subject": "Add Day138 project folder organization dry-run inventory gate"},
    {"hash": "79fc181", "subject": "Add Day137 project folder organization decision gate"},
    {"hash": "a8de0c8", "subject": "Add Pre-Day137-2 project folder organization move risk matrix"},
    {"hash": "c0dbb28", "subject": "Add Pre-Day137 project folder safety inventory no-move plan"},
    {"hash": "83c8efb", "subject": "Merge branch 'day136-ai-reviewer-export-package-integration'"},
)

DAY144_FROZEN_STATEMENT = "Day144 is frozen input only and was not rerun, rewritten, repaired, or modified."
NO_FOLDER_MOVE_STATEMENT = "Day145 performs no folder move, rename, relocation, or reorganization."
NO_CLEANUP_STATEMENT = "Day145 performs no cleanup and does not run broad cleanup commands such as git clean."
NO_PROVIDER_API_MODEL_STATEMENT = "Day145 does not call providers, APIs, OpenAI API, or models."
NO_SSH_REAL_DEVICE_STATEMENT = "Day145 does not use SSH, NETCONF, RESTCONF, RouterOS, live devices, or real network access."
NO_PHASE_ADVANCE_STATEMENT = "Day145 does not advance into execution, provider, API, or model execution phase."
FREEZE_REFERENCE_STATEMENT = f"Day145 freeze reference commit is {FREEZE_COMMIT_HASH}."

REQUIRED_TRUE_FIELDS: Tuple[str, ...] = (
    "review_only",
    "report_only",
    "evidence_only",
    "local_only",
    "deterministic_static_data_only",
    "day127_day144_scope_included",
    "day144_frozen_input_only",
    "day144_untouched",
    "freeze_reference_recorded",
    "no_folder_move",
    "no_cleanup",
    "no_provider_api_model_execution",
    "no_ssh_or_real_device_access",
    "no_execution_provider_api_phase_advance",
)

REQUIRED_FALSE_FIELDS: Tuple[str, ...] = (
    "execution_allowed",
    "provider_allowed",
    "api_allowed",
    "openai_api_called",
    "ai_provider_called",
    "model_invocation_allowed",
    "external_ai_runtime_allowed",
    "execution_runner_behavior_added",
    "adapter_execution_allowed",
    "broker_execution_allowed",
    "runner_execution_allowed",
    "live_device_access_allowed",
    "real_device_access_allowed",
    "live_network_access_allowed",
    "ssh_allowed",
    "netconf_allowed",
    "restconf_allowed",
    "routeros_allowed",
    "configuration_change_allowed",
    "configuration_changing_commands_allowed",
    "reset_reboot_remove_disable_enable_allowed",
    "secrets_allowed",
    "credentials_allowed",
    "environment_provider_activation_allowed",
    "folder_move_performed",
    "folders_moved",
    "folder_reorganization_performed",
    "cleanup_performed",
    "broad_cleanup_command_run",
    "git_clean_run",
    "day144_modified",
    "day144_reports_modified",
    "day144_docs_modified",
    "day144_roadmap_modified",
    "day144_evidence_modified",
    "day144_rerun",
    "day144_rewritten",
    "day144_repaired",
    "execution_provider_api_phase_advanced",
    "next_phase_allowed",
)

REQUIRED_REPORT_FIELDS: Tuple[str, ...] = (
    "day",
    "day_label",
    "task",
    "title",
    "mode",
    "overall_status",
    "agents_md_pre_read_result",
    "agents_md_read_before_day145_work",
    "freeze_scope",
    "freeze_commit_hash",
    "source_artifact_count",
    "day144_frozen_artifacts",
    "freeze_checks",
    "final_recommendation",
)

SOURCE_ARTIFACTS: Tuple[Dict[str, Any], ...] = (
    {
        "day": "Day127",
        "name": "AI Reviewer Summary Schema Contract",
        "paths": (
            "intent_ai_reviewer_summary_schema_contract.py",
            "docs/ai-intent/day127_ai_reviewer_summary_schema_contract.md",
            "docs/roadmap/day127_ai_reviewer_summary_schema_contract.md",
            "reports/lab-summary/day127_ai_reviewer_summary_schema_contract.json",
            "reports/lab-summary/day127_ai_reviewer_summary_schema_contract.html",
            "fixtures/day127_ai_reviewer_summary.example.json",
        ),
    },
    {
        "day": "Day128",
        "name": "AI Reviewer Summary Fixture Renderer",
        "paths": (
            "intent_ai_reviewer_summary_fixture_renderer.py",
            "docs/ai-intent/day128_ai_reviewer_summary_fixture_renderer.md",
            "docs/roadmap/day128_ai_reviewer_summary_fixture_renderer.md",
            "reports/lab-summary/day128_ai_reviewer_summary_fixture_renderer.json",
            "reports/lab-summary/day128_ai_reviewer_summary_fixture_renderer.html",
            "reports/lab-summary/day128_ai_reviewer_summary_fixture_renderer.txt",
        ),
    },
    {
        "day": "Day129",
        "name": "AI Summary Prompt Contract",
        "paths": (
            "intent_ai_summary_prompt_contract.py",
            "docs/ai-intent/day129_ai_summary_prompt_contract.md",
            "docs/roadmap/day129_ai_summary_prompt_contract.md",
            "reports/lab-summary/day129_ai_summary_prompt_contract.json",
            "reports/lab-summary/day129_ai_summary_prompt_contract.html",
        ),
    },
    {
        "day": "Day130",
        "name": "AI Summary Redaction and No-Secret Policy",
        "paths": (
            "intent_ai_summary_redaction_policy.py",
            "docs/ai-intent/day130_ai_summary_redaction_and_no_secret_policy.md",
            "docs/roadmap/day130_ai_summary_redaction_and_no_secret_policy.md",
            "reports/lab-summary/day130_ai_summary_redaction_and_no_secret_policy.json",
            "reports/lab-summary/day130_ai_summary_redaction_and_no_secret_policy.html",
            "fixtures/day130_ai_summary_redaction_policy.example.json",
        ),
    },
    {
        "day": "Day131",
        "name": "AI Summary Audit Trail Binding",
        "paths": (
            "intent_ai_summary_audit_trail_binding.py",
            "docs/ai-intent/day131_ai_summary_audit_trail_binding.md",
            "docs/roadmap/day131_ai_summary_audit_trail_binding.md",
            "reports/lab-summary/day131_ai_summary_audit_trail_binding.json",
            "reports/lab-summary/day131_ai_summary_audit_trail_binding.html",
        ),
    },
    {
        "day": "Day132",
        "name": "AI Summary Dashboard Card Integration",
        "paths": (
            "intent_ai_summary_dashboard_card_integration.py",
            "docs/ai-intent/day132_ai_summary_dashboard_card_integration.md",
            "docs/roadmap/day132_ai_summary_dashboard_card_integration.md",
            "reports/lab-summary/day132_ai_summary_dashboard_card_integration.json",
            "reports/lab-summary/day132_ai_summary_dashboard_card_integration.html",
        ),
    },
    {
        "day": "Day133",
        "name": "Disabled AI Provider Interface Boundary",
        "paths": (
            "disabled_ai_provider_interface_boundary.py",
            "docs/ai-intent/day133_disabled_ai_provider_interface_boundary.md",
            "docs/roadmap/day133_disabled_ai_provider_interface_boundary.md",
            "reports/lab-summary/day133_disabled_ai_provider_interface_boundary.json",
            "reports/lab-summary/day133_disabled_ai_provider_interface_boundary.html",
        ),
    },
    {
        "day": "Day134",
        "name": "Disabled AI Provider Adapter Contract",
        "paths": (
            "disabled_ai_provider_adapter_contract.py",
            "docs/ai-intent/day134_disabled_ai_provider_adapter_contract.md",
            "docs/roadmap/day134_disabled_ai_provider_adapter_contract.md",
            "reports/lab-summary/day134_disabled_ai_provider_adapter_contract.json",
            "reports/lab-summary/day134_disabled_ai_provider_adapter_contract.html",
        ),
    },
    {
        "day": "Day135",
        "name": "AI Provider Disabled-by-Default Safety Regression",
        "paths": (
            "ai_provider_disabled_by_default_safety_regression.py",
            "docs/ai-intent/day135_ai_provider_disabled_by_default_safety_regression.md",
            "docs/roadmap/day135_ai_provider_disabled_by_default_safety_regression.md",
            "reports/lab-summary/day135_ai_provider_disabled_by_default_safety_regression.json",
            "reports/lab-summary/day135_ai_provider_disabled_by_default_safety_regression.html",
        ),
    },
    {
        "day": "Day136",
        "name": "AI Reviewer Export Package Integration",
        "paths": (
            "ai_reviewer_export_package_integration.py",
            "docs/roadmap/day136_ai_reviewer_export_package_integration.md",
            "reports/lab-summary/day136_ai_reviewer_export_package_integration.json",
            "reports/lab-summary/day136_ai_reviewer_export_package_integration.html",
        ),
    },
    {
        "day": "Day137",
        "name": "Project Folder Organization Decision Gate",
        "paths": (
            "project_folder_organization_decision_gate.py",
            "docs/roadmap/day137_project_folder_organization_decision_gate.md",
            "reports/lab-summary/day137_project_folder_organization_decision_gate.json",
            "reports/lab-summary/day137_project_folder_organization_decision_gate.html",
        ),
    },
    {
        "day": "Day138",
        "name": "Project Folder Organization Dry-Run Inventory Gate",
        "paths": (
            "intent_project_folder_organization_dry_run_inventory_gate.py",
            "docs/ai-intent/day138_project_folder_organization_dry_run_inventory_gate.md",
            "docs/roadmap/day138_project_folder_organization_dry_run_inventory_gate.md",
            "reports/lab-summary/day138_project_folder_organization_dry_run_inventory_gate.json",
            "reports/lab-summary/day138_project_folder_organization_dry_run_inventory_gate.html",
        ),
    },
    {
        "day": "Day139",
        "name": "Docs-Only Move Dry-Run Evidence Plan",
        "paths": (
            "intent_docs_only_move_dry_run_evidence_plan.py",
            "docs/ai-intent/day139_docs_only_move_dry_run_evidence_plan.md",
            "docs/roadmap/day139_docs_only_move_dry_run_evidence_plan.md",
            "reports/lab-summary/day139_docs_only_move_dry_run_evidence_plan.json",
            "reports/lab-summary/day139_docs_only_move_dry_run_evidence_plan.html",
        ),
    },
    {
        "day": "Day140",
        "name": "Folder Move Compatibility Gate",
        "paths": (
            "intent_folder_move_compatibility_gate.py",
            "docs/ai-intent/day140_folder_move_compatibility_gate.md",
            "docs/roadmap/day140_folder_move_compatibility_gate.md",
            "reports/lab-summary/day140_folder_move_compatibility_gate.json",
            "reports/lab-summary/day140_folder_move_compatibility_gate.html",
        ),
    },
    {
        "day": "Day141",
        "name": "AI Assistance Review Demo Package",
        "paths": (
            "day141_ai_assistance_review_demo_package.py",
            "docs/ai-intent/day141_ai_assistance_review_demo_package.md",
            "docs/roadmap/day141_ai_assistance_review_demo_package.md",
            "reports/lab-summary/day141_ai_assistance_review_demo_package.json",
            "reports/lab-summary/day141_ai_assistance_review_demo_package.html",
        ),
    },
    {
        "day": "Day142",
        "name": "AI Summary to Dry-run Draft Display Contract",
        "paths": (
            "day142_ai_summary_to_dry_run_draft_display_contract.py",
            "docs/ai-intent/day142_ai_summary_to_dry_run_draft_display_contract.md",
            "docs/roadmap/day142_ai_summary_to_dry_run_draft_display_contract.md",
            "reports/lab-summary/day142_ai_summary_to_dry_run_draft_display_contract.json",
            "reports/lab-summary/day142_ai_summary_to_dry_run_draft_display_contract.html",
        ),
    },
    {
        "day": "Day143",
        "name": "Dry-run Draft Safety Diff Viewer",
        "paths": (
            "intent_dry_run_draft_safety_diff_viewer.py",
            "docs/ai-intent/day143_dry_run_draft_safety_diff_viewer.md",
            "docs/roadmap/day143_dry_run_draft_safety_diff_viewer.md",
            "reports/lab-summary/day143_dry_run_draft_safety_diff_viewer.json",
            "reports/lab-summary/day143_dry_run_draft_safety_diff_viewer.html",
            "fixtures/day143_baseline_dry_run_draft_display_payload.example.json",
            "fixtures/day143_candidate_dry_run_draft_display_payload.example.json",
        ),
    },
    {
        "day": "Day144",
        "name": "v0.4 AI Assistance Compatibility Review",
        "paths": (
            "day144_v04_ai_assistance_compatibility_review.py",
            "docs/ai-intent/day144_v04_ai_assistance_compatibility_review.md",
            "docs/roadmap/day144_v04_ai_assistance_compatibility_review.md",
            "reports/lab-summary/day144_v04_ai_assistance_compatibility_review.json",
            "reports/lab-summary/day144_v04_ai_assistance_compatibility_review.html",
        ),
        "frozen_input_only": True,
    },
)


def build_agents_md_evidence(project_root: Path) -> Dict[str, Any]:
    agents_path = Path(project_root) / "AGENTS.md"
    try:
        contents = agents_path.read_text(encoding="utf-8")
    except OSError as exc:
        return {
            "agents_md_pre_read_result": FAIL_STATUS,
            "agents_md_read_before_day145_work": False,
            "agents_md_modified": False,
            "agents_md_path": "AGENTS.md",
            "agents_md_status": f"READ_ERROR: {exc}",
        }

    markers_present = "Core Safety Rules" in contents and "Standard Validation" in contents
    return {
        "agents_md_pre_read_result": OVERALL_STATUS if markers_present else FAIL_STATUS,
        "agents_md_read_before_day145_work": markers_present,
        "agents_md_modified": False,
        "agents_md_path": "AGENTS.md",
        "agents_md_status": "FOUND_AND_READ" if markers_present else "FOUND_WITHOUT_REQUIRED_MARKERS",
    }


def build_source_artifact_records(project_root: Path) -> list[Dict[str, Any]]:
    root = Path(project_root)
    records: list[Dict[str, Any]] = []
    for index, artifact in enumerate(SOURCE_ARTIFACTS, start=1):
        path_records = [_build_path_record(root, path) for path in artifact["paths"]]
        records.append(
            {
                "artifact_id": f"DAY145_FREEZE_SOURCE_{index:02d}",
                "source_day": artifact["day"],
                "name": artifact["name"],
                "paths": path_records,
                "all_paths_exist": all(item["path_exists"] is True for item in path_records),
                "frozen_input_only": bool(artifact.get("frozen_input_only", False)),
                "review_mode": "static_reference_only",
                "freeze_status": "FROZEN",
                "write_target": False,
                "rerun_allowed": False,
                "rewrite_allowed": False,
                "repair_allowed": False,
                "execution_allowed": False,
                "provider_allowed": False,
                "api_allowed": False,
                "next_phase_allowed": False,
            }
        )
    return records


def _build_path_record(project_root: Path, relative_path: str) -> Dict[str, Any]:
    path = project_root / relative_path
    exists = path.exists()
    return {
        "path": relative_path,
        "path_exists": exists,
        "sha256": _sha256_file(path) if exists and path.is_file() else None,
    }


def _sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as handle:
        for chunk in iter(lambda: handle.read(65536), b""):
            digest.update(chunk)
    return digest.hexdigest()


def build_freeze_checks(source_artifacts: Iterable[Mapping[str, Any]]) -> list[Dict[str, Any]]:
    artifacts = list(source_artifacts)
    day144_artifacts = [item for item in artifacts if item.get("source_day") == "Day144"]
    return [
        {
            "check_id": "DAY145-FREEZE-001",
            "name": "Day127-Day144 included scope",
            "status": "PASS" if [item.get("source_day") for item in artifacts] == [f"Day{day}" for day in range(127, 145)] else "FAIL",
            "freeze_scope": "Day127-Day144",
            "next_phase_allowed": False,
        },
        {
            "check_id": "DAY145-FREEZE-002",
            "name": "Day144 frozen and untouched",
            "status": "PASS" if day144_artifacts and day144_artifacts[0].get("frozen_input_only") is True else "FAIL",
            "day144_modified": False,
            "day144_reports_modified": False,
            "day144_docs_modified": False,
            "day144_roadmap_modified": False,
            "day144_evidence_modified": False,
            "day144_rerun": False,
            "day144_rewritten": False,
            "day144_repaired": False,
            "next_phase_allowed": False,
        },
        {
            "check_id": "DAY145-FREEZE-003",
            "name": "No folder move and no cleanup",
            "status": "PASS",
            "folder_move_performed": False,
            "folders_moved": False,
            "folder_reorganization_performed": False,
            "cleanup_performed": False,
            "broad_cleanup_command_run": False,
            "git_clean_run": False,
        },
        {
            "check_id": "DAY145-FREEZE-004",
            "name": "Provider API model execution remains disabled",
            "status": "PASS",
            "provider_allowed": False,
            "api_allowed": False,
            "openai_api_called": False,
            "ai_provider_called": False,
            "model_invocation_allowed": False,
            "external_ai_runtime_allowed": False,
        },
        {
            "check_id": "DAY145-FREEZE-005",
            "name": "SSH real devices execution phase remains closed",
            "status": "PASS",
            "execution_allowed": False,
            "live_device_access_allowed": False,
            "real_device_access_allowed": False,
            "live_network_access_allowed": False,
            "ssh_allowed": False,
            "netconf_allowed": False,
            "restconf_allowed": False,
            "routeros_allowed": False,
            "execution_provider_api_phase_advanced": False,
            "next_phase_allowed": False,
        },
    ]


def build_day145_v04_ai_assistance_evidence_freeze_package(project_root: Path) -> Dict[str, Any]:
    source_artifacts = build_source_artifact_records(project_root)
    freeze_checks = build_freeze_checks(source_artifacts)
    day144_frozen_artifacts = [item for item in source_artifacts if item["source_day"] == "Day144"]
    report: Dict[str, Any] = {
        "day": DAY,
        "day_label": DAY_LABEL,
        "task": TASK_NAME,
        "title": TITLE,
        "full_title": FULL_TITLE,
        "mode": MODE,
        "overall_status": "PENDING",
        "status": "PENDING",
        **build_agents_md_evidence(project_root),
        **{field: True for field in REQUIRED_TRUE_FIELDS},
        **{field: False for field in REQUIRED_FALSE_FIELDS},
        "freeze_scope": "Day127-Day144",
        "included_day_range": [f"Day{day}" for day in range(127, 145)],
        "freeze_commit_hash": FREEZE_COMMIT_HASH,
        "initial_git_status_observed": INITIAL_GIT_STATUS_OBSERVED,
        "recent_commits_observed": list(RECENT_COMMITS_OBSERVED),
        "source_artifacts": source_artifacts,
        "source_artifact_count": len(source_artifacts),
        "source_artifact_missing_count": sum(1 for item in source_artifacts if item["all_paths_exist"] is not True),
        "day144_frozen_artifacts": day144_frozen_artifacts,
        "freeze_checks": freeze_checks,
        "explicit_boundary_statements": [
            DAY144_FROZEN_STATEMENT,
            NO_FOLDER_MOVE_STATEMENT,
            NO_CLEANUP_STATEMENT,
            NO_PROVIDER_API_MODEL_STATEMENT,
            NO_SSH_REAL_DEVICE_STATEMENT,
            NO_PHASE_ADVANCE_STATEMENT,
            FREEZE_REFERENCE_STATEMENT,
        ],
        "reviewer_next_action": "Review the Day145 freeze package; do not rerun Day144, move folders, clean up files, enable providers/APIs/models, use SSH/live devices, or unlock the next phase.",
        "final_recommendation": FINAL_RECOMMENDATION,
        "report_paths": {"json": REPORT_JSON.as_posix(), "html": REPORT_HTML.as_posix()},
        "validation_errors": [],
    }
    report["validation_errors"] = collect_validation_errors(report)
    report["overall_status"] = OVERALL_STATUS if not report["validation_errors"] else FAIL_STATUS
    report["status"] = READY_STATUS if report["overall_status"] == OVERALL_STATUS else BLOCKED_STATUS
    return report


def collect_validation_errors(report: Mapping[str, Any]) -> list[str]:
    errors: list[str] = []
    for field in REQUIRED_REPORT_FIELDS:
        if field not in report:
            errors.append(f"{field} is missing.")

    expected_values = {
        "day": DAY,
        "day_label": DAY_LABEL,
        "task": TASK_NAME,
        "title": TITLE,
        "mode": MODE,
        "freeze_scope": "Day127-Day144",
        "freeze_commit_hash": FREEZE_COMMIT_HASH,
        "final_recommendation": FINAL_RECOMMENDATION,
    }
    for field, expected in expected_values.items():
        if report.get(field) != expected:
            errors.append(f"{field} must be {expected}.")

    if report.get("agents_md_pre_read_result") != OVERALL_STATUS:
        errors.append("agents_md_pre_read_result must be PASS.")
    if report.get("agents_md_read_before_day145_work") is not True:
        errors.append("agents_md_read_before_day145_work must be true.")
    if report.get("agents_md_modified") is not False:
        errors.append("agents_md_modified must be false.")

    for field in REQUIRED_TRUE_FIELDS:
        if report.get(field) is not True:
            errors.append(f"{field} must be true.")
    for field in REQUIRED_FALSE_FIELDS:
        if report.get(field) is not False:
            errors.append(f"{field} must be false.")

    _validate_source_artifacts(report.get("source_artifacts", []), errors)
    _validate_day144_frozen_artifacts(report.get("day144_frozen_artifacts", []), errors)
    _validate_freeze_checks(report.get("freeze_checks", []), errors)
    _validate_boundary_statements(report.get("explicit_boundary_statements", []), errors)
    return errors


def _validate_source_artifacts(artifacts: Any, errors: list[str]) -> None:
    if not isinstance(artifacts, list) or len(artifacts) != len(SOURCE_ARTIFACTS):
        errors.append("source_artifacts must cover Day127-Day144.")
        return
    expected_days = [f"Day{day}" for day in range(127, 145)]
    days = [artifact.get("source_day") for artifact in artifacts if isinstance(artifact, Mapping)]
    if days != expected_days:
        errors.append("source_artifacts must be ordered Day127 through Day144.")
    for artifact in artifacts:
        if not isinstance(artifact, Mapping):
            errors.append("Each source artifact must be an object.")
            continue
        if artifact.get("all_paths_exist") is not True:
            errors.append(f"{artifact.get('artifact_id', '<unknown>')} must have all frozen paths present.")
        if artifact.get("review_mode") != "static_reference_only":
            errors.append(f"{artifact.get('artifact_id', '<unknown>')} review_mode must be static_reference_only.")
        if artifact.get("freeze_status") != "FROZEN":
            errors.append(f"{artifact.get('artifact_id', '<unknown>')} freeze_status must be FROZEN.")
        for field in (
            "write_target",
            "rerun_allowed",
            "rewrite_allowed",
            "repair_allowed",
            "execution_allowed",
            "provider_allowed",
            "api_allowed",
            "next_phase_allowed",
        ):
            if artifact.get(field) is not False:
                errors.append(f"{artifact.get('artifact_id', '<unknown>')} {field} must be false.")


def _validate_day144_frozen_artifacts(day144_artifacts: Any, errors: list[str]) -> None:
    if not isinstance(day144_artifacts, list) or len(day144_artifacts) != 1:
        errors.append("day144_frozen_artifacts must contain exactly one Day144 artifact record.")
        return
    artifact = day144_artifacts[0]
    if not isinstance(artifact, Mapping):
        errors.append("day144_frozen_artifacts entry must be an object.")
        return
    if artifact.get("source_day") != "Day144":
        errors.append("day144_frozen_artifacts must reference Day144.")
    if artifact.get("frozen_input_only") is not True:
        errors.append("Day144 frozen artifact must be frozen_input_only.")
    for field in ("write_target", "rerun_allowed", "rewrite_allowed", "repair_allowed", "next_phase_allowed"):
        if artifact.get(field) is not False:
            errors.append(f"Day144 frozen artifact {field} must be false.")


def _validate_freeze_checks(checks: Any, errors: list[str]) -> None:
    if not isinstance(checks, list) or len(checks) != 5:
        errors.append("freeze_checks must contain five Day145 checks.")
        return
    for check in checks:
        if not isinstance(check, Mapping):
            errors.append("Each freeze check must be an object.")
            continue
        if check.get("status") != "PASS":
            errors.append(f"{check.get('check_id', '<unknown>')} status must be PASS.")
        for field, value in check.items():
            if field in REQUIRED_FALSE_FIELDS and value is not False:
                errors.append(f"{check.get('check_id', '<unknown>')} {field} must be false.")


def _validate_boundary_statements(statements: Any, errors: list[str]) -> None:
    required = {
        DAY144_FROZEN_STATEMENT,
        NO_FOLDER_MOVE_STATEMENT,
        NO_CLEANUP_STATEMENT,
        NO_PROVIDER_API_MODEL_STATEMENT,
        NO_SSH_REAL_DEVICE_STATEMENT,
        NO_PHASE_ADVANCE_STATEMENT,
        FREEZE_REFERENCE_STATEMENT,
    }
    if not isinstance(statements, list) or not required.issubset(set(statements)):
        errors.append("explicit_boundary_statements must include all Day145 boundary statements.")


def write_day145_v04_ai_assistance_evidence_freeze_package_reports(
    project_root: Path,
    report: Optional[Dict[str, Any]] = None,
) -> Tuple[Path, Path]:
    safe_report = (
        deepcopy(report)
        if report is not None
        else build_day145_v04_ai_assistance_evidence_freeze_package(project_root)
    )
    json_path = Path(project_root) / REPORT_JSON
    html_path = Path(project_root) / REPORT_HTML
    json_path.parent.mkdir(parents=True, exist_ok=True)
    json_path.write_text(json.dumps(safe_report, indent=2), encoding="utf-8")
    write_day145_v04_ai_assistance_evidence_freeze_package_html(safe_report, html_path)
    return json_path, html_path


def write_day145_v04_ai_assistance_evidence_freeze_package_html(
    report: Mapping[str, Any],
    output_path: Path,
) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    summary_rows = _table_rows((field, report[field]) for field in REQUIRED_REPORT_FIELDS if field in report)
    flag_rows = _table_rows((field, report[field]) for field in REQUIRED_TRUE_FIELDS + REQUIRED_FALSE_FIELDS)
    artifact_rows = _table_rows(
        (
            item.get("source_day", ""),
            item.get("name", ""),
            item.get("freeze_status", ""),
            item.get("all_paths_exist", False),
            item.get("frozen_input_only", False),
            len(item.get("paths", [])),
        )
        for item in report.get("source_artifacts", [])
    )
    check_rows = _table_rows(
        (
            item.get("check_id", ""),
            item.get("name", ""),
            item.get("status", ""),
            item.get("next_phase_allowed", False),
        )
        for item in report.get("freeze_checks", [])
    )
    commit_rows = _table_rows(
        (item.get("hash", ""), item.get("subject", ""))
        for item in report.get("recent_commits_observed", [])
    )
    output_path.write_text(
        f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>{html.escape(str(report['full_title']))}</title>
  <style>
    body {{ font-family: Arial, sans-serif; margin: 2rem; color: #17202a; }}
    table {{ border-collapse: collapse; width: 100%; margin: 1rem 0 2rem; }}
    th, td {{ border: 1px solid #d5d8dc; padding: 0.55rem; text-align: left; vertical-align: top; }}
    th {{ background: #eef2f6; }}
    code {{ background: #eef2f6; padding: 1px 4px; border-radius: 3px; }}
  </style>
</head>
<body>
  <h1>{html.escape(str(report['full_title']))}</h1>
  <p><strong>{html.escape(DAY144_FROZEN_STATEMENT)}</strong></p>
  <p><strong>{html.escape(NO_FOLDER_MOVE_STATEMENT)}</strong></p>
  <p><strong>{html.escape(NO_CLEANUP_STATEMENT)}</strong></p>
  <p><strong>{html.escape(NO_PROVIDER_API_MODEL_STATEMENT)}</strong></p>
  <p><strong>{html.escape(NO_SSH_REAL_DEVICE_STATEMENT)}</strong></p>
  <p><strong>{html.escape(NO_PHASE_ADVANCE_STATEMENT)}</strong></p>
  <p><strong>{html.escape(FREEZE_REFERENCE_STATEMENT)}</strong></p>
  <h2>Summary</h2>
  <table><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody>{summary_rows}</tbody></table>
  <h2>Freeze Checks</h2>
  <table><thead><tr><th>ID</th><th>Name</th><th>Status</th><th>Next Phase Allowed</th></tr></thead><tbody>{check_rows}</tbody></table>
  <h2>Safety Flags</h2>
  <table><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody>{flag_rows}</tbody></table>
  <h2>Frozen Source Artifacts</h2>
  <table><thead><tr><th>Day</th><th>Name</th><th>Status</th><th>All Paths Exist</th><th>Frozen Input Only</th><th>Path Count</th></tr></thead><tbody>{artifact_rows}</tbody></table>
  <h2>Observed Recent Commits</h2>
  <table><thead><tr><th>Hash</th><th>Subject</th></tr></thead><tbody>{commit_rows}</tbody></table>
</body>
</html>
""",
        encoding="utf-8",
    )


def run_day145_v04_ai_assistance_evidence_freeze_package(
    project_root: Path,
    format_heading_func: Optional[Callable[[str], str]] = None,
    format_status_func: Optional[Callable[[str], str]] = None,
    relative_to_project_func: Optional[Callable[[Path, Path], str]] = None,
) -> int:
    report = build_day145_v04_ai_assistance_evidence_freeze_package(project_root)
    json_path, html_path = write_day145_v04_ai_assistance_evidence_freeze_package_reports(project_root, report)
    format_heading = format_heading_func or (lambda text: text)
    format_status = format_status_func or (lambda status: f"[{status}]")
    relative_to_project = relative_to_project_func or _default_relative_to_project

    print(f"AGENTS.md pre-read: {report['agents_md_pre_read_result']}")
    print(f"AGENTS.md read before Day145 work: {json.dumps(report['agents_md_read_before_day145_work'])}")
    print(f"AGENTS.md modified: {json.dumps(report['agents_md_modified'])}")
    print(format_heading(FULL_TITLE))
    print(f"Day145 task: {TITLE}")
    print(f"Task slug: {TASK_NAME}")
    for statement in report["explicit_boundary_statements"]:
        print(statement)
    print(f"freeze_scope: {json.dumps(report['freeze_scope'])}")
    print(f"freeze_commit_hash: {json.dumps(report['freeze_commit_hash'])}")
    print(f"initial_git_status_observed: {json.dumps(report['initial_git_status_observed'])}")
    print(f"source_artifact_count: {report['source_artifact_count']}")
    print(f"source_artifact_missing_count: {report['source_artifact_missing_count']}")
    for field in REQUIRED_TRUE_FIELDS + REQUIRED_FALSE_FIELDS:
        print(f"{field}: {json.dumps(report[field])}")
    print(f"final_recommendation: {json.dumps(report['final_recommendation'])}")
    print(f"JSON report: {relative_to_project(project_root, json_path)}")
    print(f"HTML report: {relative_to_project(project_root, html_path)}")

    if report["overall_status"] == OVERALL_STATUS and not report["validation_errors"]:
        print(f"{format_status(OVERALL_STATUS)} {READY_STATUS}")
        return 0

    print(f"{format_status(FAIL_STATUS)} {BLOCKED_STATUS}")
    return 1


def _default_relative_to_project(project_root: Path, path: Path) -> str:
    try:
        return Path(path).resolve().relative_to(Path(project_root).resolve()).as_posix()
    except ValueError:
        return Path(path).as_posix()


def _cell_text(value: Any) -> str:
    if isinstance(value, bool):
        return json.dumps(value)
    if isinstance(value, (dict, list)):
        return json.dumps(value, sort_keys=True)
    return str(value)


def _table_rows(rows: Iterable[Iterable[Any]]) -> str:
    return "".join(
        "<tr>" + "".join(f"<td>{html.escape(_cell_text(value))}</td>" for value in row) + "</tr>"
        for row in rows
    )
