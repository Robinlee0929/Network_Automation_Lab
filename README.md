...
| Lab topology summary | `topology_profiles/day6_lab_topology.json` | `python day6_lab_topology_summary.py` | `reports/day6_lab_topology_summary.html` |
...
Build the Day 6 profile-based lab topology summary after MikroTik Day 4 and Cisco Day 5 reports exist:

```powershell
python day6_lab_topology_summary.py
python day6_lab_topology_summary.py --profile topology_profiles/day6_lab_topology.json
```

The Day 6 compatibility alias also works:
...
At runtime, the script asks you to confirm the Cisco switch host/IP:

```text
Please input Cisco switch host/IP (press Enter to use config default: 192.168.0.111):
```

Press Enter to use `host` from `config.cisco.json`, or type a different switch IP for the current run. If the profile's `expected_management_ip` still matches the old host, the script updates it for this run so the Vlan1 management IP check follows the switch IP you entered.
...
## Day 6 Profile-Based Lab Topology Summary

Day 6 aggregates the existing device-level reports into one portfolio-friendly lab topology report. It does not connect to routers or switches. It only reads JSON reports already generated by earlier workflows, so it is safe to run offline and in CI.
...
```powershell
python day6_lab_topology_summary.py --profile topology_profiles/day6_lab_topology.json
python topology_summary.py --profile topology_profiles/day6_lab_topology.json
```

The topology profile controls which devices are included:

```json
{
  "topology_name": "day6_lab_topology",
  "devices": [
    {
      "device_name": "cisco-switch",
      "device_type": "cisco_ios",
      "role": "core_switch",
      "management_ip": "192.168.0.111",
      "report_path": "reports/cisco-switch/switch_topology_report.json",
      "required": true
    }
  ]
}
...
To replace a router, switch, management IP, role, or report location, edit only the topology profile. The Day 6 Python code does not hard-code `Hex-s-2025-lab01`, `Hex-s-2025-lab02`, `cisco-switch`, or their report paths.
...
Result normalization accepts common source fields:

| Source field | Normalized value |
| --- | --- |
| `overall_result` | `PASS`, `FAIL`, or `UNKNOWN` |
| `overall_status` | `PASS`, `FAIL`, or `UNKNOWN` |
| `result` | `PASS`, `FAIL`, or `UNKNOWN` |
| `status` | `PASS`, `FAIL`, or `UNKNOWN` |

If no supported result field exists, Day 6 reports `UNKNOWN` and does not treat it as `PASS`.

Overall lab result rules:

| Overall | Condition |
| --- | --- |
| `PASS` | Every required device report exists and is `PASS`. |
| `FAIL` | Any required device report is missing or `FAIL`. |
| `WARNING` | An optional report is missing, or a device result is `UNKNOWN` while no required device has failed. |
...
The HTML report is intended for portfolio screenshots and project demos. It highlights the topology name, lab health, required/optional coverage, missing reports, failed checks, source report type, and the profile-driven device inventory.
It also expands per-device test evidence when source reports include detailed items under fields such as `checks`, `validation_results`, `test_results`, `summary.checks`, or `validations`.
