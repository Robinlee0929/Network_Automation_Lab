...
Phase 2M-06 is `DONE / READY_FOR_REVIEW` on its local feature branch. It adds exactly one GitHub Actions workflow at `.github/workflows/safe-ci.yml` for pull requests targeting `main`, pushes to `main`, and manual dispatch. The single GitHub-hosted Ubuntu job has `contents: read` permission, disables persisted checkout credentials, references no secret, and pins checkout, Node setup, and Python setup actions to reviewed immutable commit SHAs with their upstream versions noted in comments.

The workflow materializes only the committed Node and Python dependency sets, then runs typecheck, zero-warning lint, the complete Node-only Vitest suite, a telemetry-disabled Next.js build, full pytest, report-index, and `git diff --exit-code`. Local validation passes with 56 Vitest tests, a 24/24-page build, and 1,866 pytest tests; report-index exits 0 with only the 13 documented optional local reports missing. No push occurred, so no GitHub-hosted workflow run or CI PASS is claimed. No source, test, dependency, configuration, server, browser, secret, live-device, provider/API/model, deployment, Phase 2M-07, or Phase 2N work was added. Detailed evidence is in `docs/phase_2m/phase_2m_06_github_actions_dual_stack_safe_ci_baseline.md`.
...
| 49 | 2M-06 | GitHub Actions Dual-Stack Safe CI Baseline | DONE / READY_FOR_REVIEW | One read-only-permissions, no-secrets dual-stack workflow is implemented locally with immutable action pins; local validation passed; no push, merge, GitHub-hosted run, or CI PASS is claimed |
