...
local-approval-envelope-validation-job writes the Phase 2J-05 local_approval_envelope_validation_job report; it validates static local approval-envelope documentation markers only and does not execute approval, grant runtime permission, add runner, adapter, scheduler, queue, broker, worker, agent loop, SSH, NETCONF, RESTCONF, live device, provider/API/model, secret, backup, config change, Day1-Day160 replacement, AGENTS.md modification, or second safety matrix.
...
def _build_task_handlers(args: argparse.Namespace, root: Path, lab: ModuleType) -> Dict[str, Any]:
    return {
        ...
        lab.PHASE_2J_05_LOCAL_APPROVAL_ENVELOPE_VALIDATION_JOB_TASK_ID: lambda: lab._run_phase_2j_05_local_approval_envelope_validation_job(root),
