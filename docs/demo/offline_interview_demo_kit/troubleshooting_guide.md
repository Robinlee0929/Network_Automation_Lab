...
Use this guide when the interview environment is missing local dependencies, generated evidence, internet, browser access, or lab devices.
...
| Likely cause | Python dependencies are not installed on the interview computer, or another process is using port `5000`. |
...
| Fallback explanation for interview | "The dashboard is a local evidence browser. The important architecture is the report model, route safety, and local evidence navigation; it does not need live devices." |
...
| Fallback explanation for interview | "Generated reports are intentionally local working evidence and may not be committed. The demo can still show the report paths, safety metadata, and evidence design." |
...
| Fallback explanation for interview | "The test suite is local and non-live. If this machine is missing a dependency, I can still explain which behavior the tests cover and why hidden live dependencies are avoided." |
...
| Fallback explanation for interview | "WARN is acceptable here when optional historical generated reports are absent. The index is honest about missing evidence instead of pretending it exists." |
...
| Safe recovery action | Use committed roadmap, portfolio, summary, topology, and runner metadata files. Do not create live evidence during the interview. |
| Fallback explanation for interview | "The committed repository demonstrates the architecture and workflow. Generated reports are normally produced by safe local or controlled lab runs, but they are not required for this offline interview demo." |
...
| Fallback explanation for interview | "The browser is only a presentation surface. The same evidence model is visible from committed Markdown, JSON metadata, and tests." |
...
| Likely cause | Offline interview environment or restricted network. |
...
| Fallback explanation for interview | "The offline kit was created specifically so the demo does not depend on GitHub or internet access." |
...
| Likely cause | The interview environment is intentionally offline or not connected to the lab. |
...
| Fallback explanation for interview | "Live testing is intentionally separated from the interview demo flow. The demo focuses on architecture, safety, repeatability, and evidence design without touching devices." |
