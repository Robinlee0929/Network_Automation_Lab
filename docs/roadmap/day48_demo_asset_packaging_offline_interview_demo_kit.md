...
Day48 packages a self-contained offline interview demo kit for Network Automation Lab.
...
- Create an offline interview demo kit under `docs/demo/offline_interview_demo_kit/`.
- Provide a demo checklist, safe command list, troubleshooting guide, interview talk track, no-live-dependency statement, and screenshot backup plan.
- Reference the Day47 interview demo baseline runbook.
...
- No GitHub, internet, or cloud dependency during the interview demo.
- No new runner behavior, dashboard behavior, live workflow, or device-control logic.

## Offline Demo Assumptions

- The repository has been cloned or copied to the interview computer before the demo.
- Python and project dependencies are already available, or the interviewer accepts a code and document walkthrough without executing commands.
...
- Confirm the dashboard can start locally if the interview environment allows it.
...
## 3-5 Minute Interview Talk Track
...
The Day48 offline interview demo does not require live router or switch access. It does not require SSH. It does not require GitHub, internet, VPN, WireGuard peers, iperf3 endpoints, or lab devices.

The demo is intentionally based on committed source code, committed documentation, local tests, local report-index generation, local dashboard routes, and existing evidence references. Live testing remains separated from the interview flow for safety.
...
During the interview, use only the safe local commands listed in the kit. Do not run VRRP live validation, WireGuard live execution, SSH-based validation, iperf3 performance tests, or device-changing tasks.
...
Day48 adds a committed offline interview demo kit and roadmap note so the project can be presented safely without live infrastructure.

The kit gives the interviewer a predictable open order, safe command set, troubleshooting path, Traditional Chinese talk track, and explicit no-live-dependency statement. It builds on Day47 by turning the demo runbook into portable offline assets.
